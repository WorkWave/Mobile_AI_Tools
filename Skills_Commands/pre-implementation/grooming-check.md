...
and produces a grooming preparation package as a Confluence page.
...
## Step 1 — Collect Story Number and Confluence Destination

If the story number was not provided at invocation, ask the user for it now.

Then ask the user for the Confluence destination:
- **Space key** (e.g. `MOB`, `PPM`)
- **Parent page name** under which the new page should be created

Use the Atlassian MCP server to fetch the full story (title, description, AC, labels, status).
...
## Step 8 — Create Confluence Page

Create a Confluence page via the Atlassian MCP server in the space and under the parent page provided by the user.

**Page title:** `Grooming Check — [Story Number]: [Story Title]`
...
After the page is created, respond with the Confluence page URL so the user can navigate directly to it.

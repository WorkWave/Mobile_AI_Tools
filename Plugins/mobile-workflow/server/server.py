...
async def call_tool(name: str, arguments: dict) -> list[types.TextContent]:
    ...
    # ── analyze image and prepare for generation ──────────────────────────────
        ...
        instruction_lines = [
            ...
            "",
            "   SCROLL VIEW CONSTRAINTS — follow these rules exactly whenever a",
            "   UIScrollView is present (vertical-scroll forms, feeds, detail screens):",
            "   a. Place a single content UIView as the only direct subview.",
            "   b. In the Layout JSON, reference the layout guides using these IDs:",
            "        '{scrollViewId}_contentLayoutGuide'  ← defines scroll content size",
            "        '{scrollViewId}_frameLayoutGuide'    ← matches the visible viewport",
            "      where {scrollViewId} is the 'id' field of the UIScrollView.",
            "   c. The content UIView MUST have exactly these 6 constraints:",
            "        top      = contentLayoutGuide.top",
            "        leading  = contentLayoutGuide.leading",
            "        trailing = contentLayoutGuide.trailing",
            "        bottom   = contentLayoutGuide.bottom",
            "        width    = frameLayoutGuide.width           ← prevents horizontal scroll",
            "        height   = frameLayoutGuide.height priority:250  ← Apple-canonical soft constraint",
            "   d. The height constraint MUST use priority 250 (low). This is Apple's",
            "      recommended pattern from the Auto Layout guide: the low priority makes it",
            "      a soft 'fill the viewport' hint, allowing content to grow taller and scroll.",
            "      A hard height = frameLayoutGuide.height at priority 1000 pins content to",
            "      exactly the viewport height and breaks scrolling entirely.",
            "      In Layout JSON: { \"attribute\": \"height\", \"to\": \"{scrollViewId}_frameLayoutGuide\",",
            "        \"toAttribute\": \"height\", \"relation\": \"equal\", \"priority\": 250 }",
            "",
